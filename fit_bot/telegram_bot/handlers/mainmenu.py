...
def restart(message):
    ...
    bot.send_photo(chat_id, photo=photo, caption=text, reply_markup=markup, parse_mode='Markdown')
...
def start_message(message: Message):
    ...
    try:
        ...
        if not paid_user.has_finished and unpaid_user.has_paid:
            paid_user_main_menu(message)
            return
        # elif not paid_user and user.has_paid:
        #     bot.set_state(user_id, AfterPurchaseStates.initial, chat_id)
...
def paid_user_main_menu(message: Message, user_id=None, chat_id=None):
    if user_id is None:
