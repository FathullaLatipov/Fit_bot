...
class Content(models.Model):
    ...
    photo = models.ImageField(upload_to='photos_official/', blank=True, null=True)
    gif = models.FileField(upload_to='gifs/', validators=[FileExtensionValidator(allowed_extensions=['gif'])],
...
class ProgramMap(models.Model):
    image = models.FileField(upload_to='static/program_images', verbose_name='Изображения')
    ...
    if created:
        ...
        if instance.photo:
            with open(instance.photo.path, 'rb') as photo_file:
                message = bot.send_photo(chat_id=58790442, photo=photo_file)
                instance.photo_file_id = message.photo[-1].file_id
                instance.photo.delete(save=False)

        if instance.gif:
            with open(instance.gif.path, 'rb') as gif_file:
                message = bot.send_document(chat_id=58790442, document=gif_file)
